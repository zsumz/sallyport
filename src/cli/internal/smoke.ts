...
export const SMOKE_DIRECTORY = 'quoin-smoke';
export const PUBLIC_SMOKE_DIRECTORY = 'quoin-public-smoke';
...
export async function runReleaseSmoke(request: SmokeRequest): Promise<SmokeOutcome> {
    ...
    try {
        const result = effects.exec('npm', ['run', 'release:smoke'], {
            ...
            env: {
                ...effects.env,
                QUOIN_TARBALL: copyPath,
                QUOIN_PACKAGE: request.packageName,
                QUOIN_VERSION: request.version,
                QUOIN_DIST_TAG: request.distTag,
            },
        });
