...
export function npmTokenCheck(context: CheckContext): CheckResult {
    ...
    if (findings.length > 0) {
        return result(
            id,
            'fail',
            `an npm publishing token appears in ${findings.join(', ')}; sallyport uses OIDC only.`,
        );
    }
...
export function directPublishCheck(context: CheckContext): CheckResult {
    ...
    if (findings.length > 0) {
        return result(
            id,
            'fail',
            `npm publish appears in ${findings.join(', ')}; publishing must go through sallyport.`,
        );
    }
    return result(id, 'pass', 'no workflow publishes outside sallyport.');
}
