...
import { SALLYPORT_WORKFLOW_SHA, isPlaceholderSha, normalizeCommitSha } from '../pins.ts';
...
export function resolveWorkflowSha(requested: string | undefined): string {
    ...
    const pinned = normalizeCommitSha(SALLYPORT_WORKFLOW_SHA);
    if (pinned === null || isPlaceholderSha(pinned)) {
        throw new Error(
            'Installation failed: this build of sallyport carries no pinned workflow commit;'
            + ' pass --sha <commit> until the first sallyport release bakes one in.'
...
export async function upgradeCallerWorkflow(
    ...
    if (existing === null) {
        throw new Error(
            ...
            + ' run npx sallyport init to generate it.',
        );
    }
    if (!hasGeneratedMarker(existing) && !force) {
        throw new Error(
            `Installation failed: ${CALLER_WORKFLOW_FILE} does not start with the sallyport`
    ...
    if (rewritten.replaced !== EXPECTED_PINS) {
        throw new Error(
            ...
            + ` found ${String(rewritten.replaced)} sallyport workflow references.`,
        );
    }
