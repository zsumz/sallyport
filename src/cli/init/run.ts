...
export async function runInit(options: InitOptions): Promise<number> {
    ...
    if (metadata.isPrivate) {
        throw new Error('Installation failed: sallyport releases public packages only.');
    }
    if (metadata.hasWorkspaces) {
        throw new Error('Installation failed: sallyport releases one root package per repository.');
    }
    ...
    log(`sallyport init ${name}@${version}`);
    log(`  repository  ${repository}`);
    log(`  profile     ${profile}`);
    log(`  sallyport sha ${sha}`);
    log('');
