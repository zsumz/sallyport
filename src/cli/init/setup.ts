...
export function setupChecklist(profile: Profile, version: string): string[] {
    const steps = [
        ...
        ...profile === 'strict'
            ? [
                'Set repository variable SALLYPORT_SIGNER_FINGERPRINT to the 40-hex primary key fingerprint.',
...
export async function scriptWarnings(dir: string): Promise<string[]> {
    ...
    return REQUIRED_SCRIPTS
        ...
        .map((script) => `package.json defines no ${script} script; sallyport cannot release without it.`);
}
