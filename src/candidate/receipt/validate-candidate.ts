...
export function validateCandidateReceipt(value: unknown): string[] {
    ...
    checksallyport(receipt, failures);
...
function checksallyport(receipt: Record<string, unknown>, failures: string[]): void {
    const fields = nested(receipt, '', 'sallyport', SALLYPORT_KEYS, failures);
    if (fields === null) return;
    checkPattern(fields, 'sallyport', 'version', patterns.semver, descriptions.semver, failures);
    checkPattern(
        fields,
        'sallyport',
        'workflow',
    ...
    checkPattern(fields, 'sallyport', 'sha', patterns.commit, descriptions.commit, failures);
}
