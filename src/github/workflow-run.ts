...
export const CALLER_WORKFLOW_PATH = '.github/workflows/quoin.yml';
...
export function validateCandidateRun(input: CandidateRunInput): string[] {
    ...
    if (receipt.quoin.sha !== expected.quoinSha) {
        failures.push(
            `Candidate quoin SHA ${receipt.quoin.sha} does not match the pinned finalizer ${expected.quoinSha}.`,
        );
    }
