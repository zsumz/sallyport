...
const SALLYPORT_SHA = '0123456789abcdef0123456789abcdef01234567';

smoke.suite('sallyport package', { tags: ['package'] }, async (t) => {
    const root = t.repoRoot();
    const work = await t.tempDir('sallyport-package');
    ...
    // Inside a sallyport release the exact candidate tarball arrives through
    // the environment; everywhere else the smoke packs its own fixture.
    const suppliedTarball = process.env.SALLYPORT_TARBALL;
    ...
    const version = process.env.SALLYPORT_VERSION
    ...
    await t.step('tarball contains the public package surface', async () => {
        await expect
            ...
            .toContainEntries([
                ...
                'package/templates/sallyport.yml',
    ...
    const cli = resolve(fixtureDir, 'node_modules', 'sallyport', 'dist', 'cli', 'main.js');
    ...
    await t.step('init generates the pinned caller workflow', async () => {
        ...
        await t.fs.writeJson(resolve(consumerDir, 'package.json'), {
            name: 'sallyport-smoke-consumer',
            version: '1.2.3',
            description: 'sallyport smoke consumer',
            repository: {
                type: 'git',
                url: 'git+https://github.com/zsumz/sallyport-smoke-consumer.git',
            },
            license: 'MIT',
            scripts: {
        ...
        await t.cmd('git', [
            ...
            'add',
            'origin',
            'git@github.com:zsumz/sallyport-smoke-consumer.git',
        ], { cwd: consumerDir });

        const init = await t.cmd('node', [cli, 'init', '--sha', SALLYPORT_SHA], {
            cwd: consumerDir,
            check: false,
        });
        if (init.exitCode !== 0) {
        ...
        const workflow = readFileSync(
            resolve(consumerDir, '.github', 'workflows', 'sallyport.yml'),
            'utf8',
        );
        if (!workflow.startsWith('# Generated by sallyport.')) {
            throw new Error('generated workflow must carry the generated-by marker');
        }
        const pins = workflow.match(/@[0-9a-f]{40}/gu) ?? [];
        if (pins.length !== 2 || pins[0] !== `@${SALLYPORT_SHA}` || pins[1] !== `@${SALLYPORT_SHA}`) {
