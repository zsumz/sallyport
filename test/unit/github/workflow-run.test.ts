...
describe('validateCandidateRun', () => {
    ...
    it('pins the generated caller workflow filename', () => {
        expect(CALLER_WORKFLOW_PATH).toBe('.github/workflows/quoin.yml');
    });
    ...
    it('rejects a run from another workflow file', () => {
        ...
        expect(failures).toStrictEqual([
            'Workflow run used .github/workflows/release.yml, expected .github/workflows/quoin.yml.',
        ]);
    });
    ...
    it('rejects a candidate built by another quoin commit', () => {
        const expected = makeExpected();
        expected.quoinSha = 'f'.repeat(40);
        const failures = validateCandidateRun({ run: makeRun(), expected });
        expect(failures).toHaveLength(1);
