...
const DIGEST = hashTarball(Buffer.from('sallyport candidate tarball', 'utf8'));
const RECEIPT_DIGEST = hashTarball(Buffer.from('sallyport candidate receipt', 'utf8'));
...
describe('buildCandidateReceipt', () => {
    ...
    it('refuses to build a receipt for a mutable workflow reference', () => {
        ...
        expect(() => buildCandidateReceipt({
            ...parts,
            sallyport: { ...parts.sallyport, sha: 'main' },
        })).toThrow(/sallyport\.sha/u);
    });
});
...
describe('validateCandidateReceipt', () => {
    ...
    it('rejects workflow references outside .github/workflows', () => {
        const receipt = buildCandidateReceipt(candidateParts());
        for (const workflow of ['stage.yml', 'zsumz/sallyport/stage.yml', 'zsumz/sallyport']) {
            expect(validateCandidateReceipt({
                ...receipt,
                sallyport: { ...receipt.sallyport, workflow },
            })).toContain(
                'sallyport.workflow must be an owner/name/.github/workflows/<file>.yml reference.',
            );
        }
    });
