...
const PACKED_FILENAME = 'quoin-fixture-1.2.3.tgz';
...
beforeEach(() => {
    root = mkdtempSync(path.join(tmpdir(), 'quoin-pack-'));
...
function arrayOutput(filename: string = PACKED_FILENAME): string {
    return JSON.stringify([{ id: 'quoin-fixture@1.2.3', filename }]);
}

function objectOutput(filename: string = PACKED_FILENAME): string {
    return JSON.stringify({ 'quoin-fixture': { id: 'quoin-fixture@1.2.3', filename } });
}
...
describe('packOnce', () => {
    ...
    it('packs a real package with npm and produces an inspectable candidate', () => {
        ...
        expect(result.reportedFilename).toBe('quoin-pack-fixture-4.5.6.tgz');
        expect(inspection.failures).toEqual([]);
        expect(inspection.name).toBe('quoin-pack-fixture');
