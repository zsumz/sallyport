...
beforeEach(() => {
    ...
    outputDir = path.join(root, 'quoin-candidate');
    runOverrides = {};
    artifactName = `quoin-candidate-${candidate.consumer.commit}`;
    zipFiles = [
...
describe('internal fetch-candidate', () => {
    ...
    it('rejects a candidate pinned to a different quoin commit', async () => {
        await expect(fetchCandidateCommand(args({ '--quoin-sha': OTHER_SHA }), withToken()))
    ...
    it('rejects an artifact named for a different commit', async () => {
        artifactName = 'quoin-candidate-deadbeef';
    ...
    it('rejects a candidate receipt that fails validation', async () => {
        ...
        await expect(fetchCandidateCommand(args(), withToken()))
            .rejects.toThrow(/protocol must be "quoin\/0.1"/u);
    });

    it('rejects a malformed quoin sha argument', async () => {
        await expect(fetchCandidateCommand(args({ '--quoin-sha': 'main' }), withToken()))
            .rejects.toThrow('--quoin-sha must be a full 40-character commit SHA.');
    });
