...
import { readQuoinVersion } from '../../../src/cli/version.ts';
import {
    createConsumer,
    createEffects,
    FIXTURE_SHA,
...
describe('runCli', () => {
    ...
    it('prints usage for every help spelling', async () => {
        ...
        expect(USAGE).toContain('quoin init [--strict] [--upgrade] [--force] [--sha <40-hex>]');
        expect(USAGE).toContain('quoin check [--json]');
    });

    it('prints the package version for every version spelling', async () => {
        const version = await readQuoinVersion();
    ...
    it('rejects an unknown command', async () => {
        await expect(runCli(['publish'], harness.effects))
            .rejects.toThrow('Unknown command publish; run quoin --help.');
    });
