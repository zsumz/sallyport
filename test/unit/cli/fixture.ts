...
export const FIXTURE_NAME = 'quoin-fixture';
...
export function makeTempRoot(label: string): string {
    return mkdtempSync(path.join(realpathSync(tmpdir()), `quoin-${label}-`));
}
...
export function createConsumer(root: string, options: ConsumerOptions = {}): Consumer {
    ...
    writeJsonFile(path.join(dir, 'package.json'), {
        name,
        version,
        description: 'quoin cli test fixture',
    ...
    writeFileSync(path.join(dir, 'index.js'), 'export const quoinFixture = true;\n');
...
const bytes = readFileSync(process.env.QUOIN_TARBALL);
if (bytes.byteLength === 0) {
    console.error('smoke: empty tarball');
    process.exit(1);
}
console.log(
    \`smoke ok \${process.env.QUOIN_PACKAGE}@\${process.env.QUOIN_VERSION}\`
    + \` \${process.env.QUOIN_DIST_TAG} \${bytes.byteLength}\`,
);
`;
