...
describe('reusable workflow privilege boundaries', () => {
    ...
    it('stage.yml:stage stages exactly one validated tarball', () => {
        ...
        expect(scripts).toContain('--tag "$QUOIN_DIST_TAG"');
    });
    ...
    it('only unprivileged jobs check out quoin implementation code', () => {
        for (const [file, job] of [['stage.yml', 'prepare'], ['finalize.yml', 'verify']] as const) {
            const quoinCheckout = stepsOf(jobOf(file, job)).find(
