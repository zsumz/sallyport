...
package `@zsumz/sallyport-fixture` has exercised stage and reject cycles,
...
Bake the reusable-workflow pin into the CLI: set `SALLYPORT_WORKFLOW_SHA` in
...
```sh
SALLYPORT_TARBALL="$PWD/sallyport-0.1.0-alpha.0.tgz" \
SALLYPORT_PACKAGE=sallyport \
SALLYPORT_VERSION=0.1.0-alpha.0 \
SALLYPORT_DIST_TAG=alpha \
...
```sh
shasum -a 256 sallyport-0.1.0-alpha.0.tgz
```

### 2. Sign

```sh
git tag --sign v0.1.0-alpha.0 -m "sallyport v0.1.0-alpha.0"
...
```sh
SALLYPORT_TARBALL="$PWD/sallyport-0.1.0-alpha.0.tgz" npm run publish:alpha
```
...
```sh
npm trust github sallyport \
  --repository zsumz/sallyport \
  --file sallyport.yml \
...
This is the `.github/workflows/sallyport.yml` caller referenced by
`--file sallyport.yml` above; it takes the place of the scaffold's
...
Release `0.1.0-alpha.1` through sallyport. From here, follow the sections below.
