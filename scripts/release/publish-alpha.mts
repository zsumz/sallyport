...
const tarball = process.env.QUOIN_TARBALL;
if (tarball === undefined || !path.isAbsolute(tarball)) {
    throw new Error('QUOIN_TARBALL must be an absolute path to the qualified tarball.');
}

const manifest = JSON.parse(await readFile(path.join(cwd, 'package.json'), 'utf8')) as Manifest;
if (manifest.name !== 'quoin' || typeof manifest.version !== 'string') {
    throw new Error('package.json must identify a versioned quoin package.');
}
...
if (workflowSha !== QUOIN_WORKFLOW_SHA) {
    throw new Error(`workflow pin ${QUOIN_WORKFLOW_SHA} must equal ${workflowSha}.`);
}
if (!dryRun) {
...
const smoke = runCommand('npm', ['run', 'release:smoke'], {
    ...
    env: {
        ...process.env,
        QUOIN_DIST_TAG: decision.distTag,
        QUOIN_PACKAGE: candidate.name,
        QUOIN_TARBALL: tarball,
        QUOIN_VERSION: candidate.version,
    },
});
